...
def reading_data(simulation:bool, design_variant:str, n_passive_runs:int) -> None:
    """
    ...
    CONDITION_SPECS = {'condition':['Additive','Multiplicative'],
                       'lambd':[0.0,1.0],
                       'bids_text': ['0d0','1d0'],
                       'txt_append':['_add','_mul']}
    SUBJECT_SPECS = sub_specs.sub_specs(design_variant, simulation)

    if simulation:
        root_path = os.path.join(os.path.dirname(__file__), 'data', design_variant, 'simulations')
    else:
        root_path = os.path.join(os.path.dirname(__file__), 'data', design_variant)
    ...
    for c,condition in enumerate(CONDITION_SPECS['condition']):
        ...
        for i,subject in enumerate(SUBJECT_SPECS['id']):
            ...
            subject_df = utils.add_info_to_df(subject_df)
...
if __name__=='__main__':
    ...
    reading_data(simulation, design_variant, n_passive_runs)