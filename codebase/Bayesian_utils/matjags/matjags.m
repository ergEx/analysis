function [samples, stats, structArray] = matjags(dataStruct, jagsFilenm, initStructs,whichMCMC,varargin)
...
% Changes added to matjags by Oliver Hulme. Function now takes whichMCMC as
% input which directs which of several copies of JAGS to run. 1st April
% 2019

if ispc
    defaultworkingDir =[ '..' filesep 'matjags\jagstmp',num2str(whichMCMC)]; %'c:\temp\jagstmp'; % change this to fit your particular needs
elseif ismac | isunix
    defaultworkingDir = [ '..' filesep 'matjags/jagstmp',num2str(whichMCMC)]; % change this to fit your particular needs
end
